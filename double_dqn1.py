...
def train(n_episodes=3000, max_t=1000, eps_start=1.0, eps_end=0.015, eps_decay=0.99, lr=0.001):
    ...
    for i_episode in range(1, n_episodes+1):
        ...
        if mean_score >= 200.0:
            print('\nSolved {:d} episodes\tAverage Score: {:.2f}'.format(
                i_episode-100, mean_score))
            torch.save(the_ship.state_dict(), 'checkpoint_win.pth')
            break
        # if i_episode >= 2000 and mean_score >= 100:
        # if i_episode >= 100 and mean_score >= 100:
        #     print('\nEnvironment aaaaaaaaaaaa {:d} episodes!\tAverage Score: {:.2f}'.format(
        #         i_episode-100, mean_score))
        #     torch.save(the_ship.state_dict(), 'checkpoint_to_train_100pts.pth')
        #     break
        if i_episode >= n_episodes - 1:
            print('\nEnvironment aaaaaaaaaaaa {:d} episodes!\tAverage Score: {:.2f}'.format(
                i_episode-100, mean_score))
            torch.save(the_ship.state_dict(), 'checkpoint_to_double_dqn_1.pth')
            break
        # if np.mean(scores_window) <= -700.0:
        #     print('\nwhat is going on {:d} episodes!\tAverage Score: {:.2f}'.format(
        #         i_episode-100, np.mean(scores_window)))
        #     torch.save(the_ship.state_dict(), 'checkpoint.pth')
        #     break
    return scores
